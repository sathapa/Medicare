...
class Assessment extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('sid, clerk, pid', 'required'),
            array('icd10code, datetime','safe'),
			array('sid, clerk, pid', 'numerical', 'integerOnly'=>true),
			array('icd10code', 'length', 'max'=>500),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('asid, sid, clerk, pid, icd10code, datetime', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('icd10code',$this->icd10code,true);
